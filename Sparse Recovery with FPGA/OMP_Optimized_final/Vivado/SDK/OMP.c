...
float *y = (float *)XPAR_AXI_EMC_0_S_AXI_MEM0_BASEADDR + m;
float *residue = (float *)XPAR_AXI_EMC_0_S_AXI_MEM0_BASEADDR + k + m;
float *A_ADDR = (float *)XPAR_AXI_EMC_0_S_AXI_MEM0_BASEADDR + 2*k + m;
float *temp = (float *)XPAR_AXI_EMC_0_S_AXI_MEM0_BASEADDR + (N+2)*k + m;   //size = 2m+k
float *x = (float *)XPAR_AXI_EMC_0_S_AXI_MEM0_BASEADDR + (N+2)*k + 2*m;
float *D = (float *)XPAR_AXI_EMC_0_S_AXI_MEM0_BASEADDR + (N+2)*k + 2*m + N;

float *maxele = (float *) 0xc0000000;
...
void leastsquare(int idx)
{
	...
	/*
	XLeastsquare_Set_S_final(&ls, m);
	XLeastsquare_Set_k_final(&ls, k);
	XLeastsquare_Start(&ls);
	Xil_DCacheFlushRange((u32)y, k*sizeof(float));
	XAxiDma_SimpleTransfer(&axidma_ls, (u32)y, k*sizeof(float), XAXIDMA_DMA_TO_DEVICE);
	while(XAxiDma_Busy(&axidma_ls, XAXIDMA_DMA_TO_DEVICE));
	*/
	//print("Least square...\n");
	int col = *(maxidx+idx-1);

	Xil_DCacheFlushRange((u32)(A_ADDR + k*col), k*sizeof(float));
	Xil_DCacheFlushRange((u32)residue, k*sizeof(float));
	XAxiDma_SimpleTransfer(&axidma_ls, (u32)(A_ADDR + k*col), k*sizeof(float), XAXIDMA_DMA_TO_DEVICE);
	while(XAxiDma_Busy(&axidma_ls, XAXIDMA_DMA_TO_DEVICE));
	//print("LS 1\n");

	XAxiDma_SimpleTransfer(&axidma_ls, (u32)residue, k*sizeof(float), XAXIDMA_DEVICE_TO_DMA);
	while(XAxiDma_Busy(&axidma_ls, XAXIDMA_DEVICE_TO_DMA));
	//print("LS 2\n");
	Xil_DCacheInvalidateRange((u32)residue, k*sizeof(float));

	if(idx==m)
	{
		Xil_DCacheFlushRange((u32)x, m*sizeof(float));
		XAxiDma_SimpleTransfer(&axidma_ls, (u32)x, m*sizeof(float), XAXIDMA_DEVICE_TO_DMA);
		while(XAxiDma_Busy(&axidma_ls, XAXIDMA_DEVICE_TO_DMA));
		Xil_DCacheInvalidateRange((u32)x, m*sizeof(float));
...
void constructImage()
{
	//print("Construct...\n");
	XMultiply_Set_row(&dot, N);
	XMultiply_Set_col(&dot, m);
	XMultiply_Start(&dot);

	Xil_DCacheFlushRange((u32)x, m*sizeof(float));
	XAxiDma_SimpleTransfer(&axidma_mult, (u32)x, m*sizeof(float), XAXIDMA_DMA_TO_DEVICE);

	for(int i=0; i<N; i++)
	{
		for(int j=0; j<m; j++)
		{
			*(temp+j) = *(D + N*i + *(maxidx+j));
		}
		Xil_DCacheFlushRange((u32)temp, m*sizeof(float));
		XAxiDma_SimpleTransfer(&axidma_mult, (u32)temp, m*sizeof(float), XAXIDMA_DMA_TO_DEVICE);

		/*Xil_DCacheFlushRange((u32)(x+i), sizeof(float));
		XAxiDma_SimpleTransfer(&axidma_mult, (u32)(x+i), sizeof(float), XAXIDMA_DEVICE_TO_DMA);
		while(XAxiDma_Busy(&axidma_mult, XAXIDMA_DEVICE_TO_DMA));
		Xil_DCacheInvalidateRange((u32)(x+i), sizeof(float));*/
	}
